 ...
 * java.util.Comparator; import java.util.Enumeration; import java.util.HashSet;
 ...
 * org.apache.commons.collections4.map.HashedMap; import
 ...
 * 
 * import in.cdacnoida.dava.daoimpl.DaoHelper; import
 * in.cdacnoida.dava.model.UserDetails; import
 * in.cdacnoida.dava.springsecurity.LoggedUser;
 * 
 * 
 *//**
 ...
 * @Component
 * 
 * @Order(2) public class FormFieldValidationFilter_local_bkp implements Filter
 * {
 * 
 * @Autowired private DaoHelper daoHelper;
 * 
 ...
 * 
 * 
 * static { keys.add("varssoticketgrantingticket"); keys.add("go.x");
 * keys.add("go.y"); }
 * 
 ...
 *//**
	 ...
	 */
/*
 * public FormFieldValidationFilter_local_bkp() { // TODO Auto-generated
 * constructor stub }
 * 
 *//**
	 * @see Filter#destroy()
 ...
 * 
 * 
 * String method = ((HttpServletRequest) request).getMethod(); if(method==null
 * method=""; String spath = ((HttpServletRequest) request).getServletPath();
 * 
 * 
 * if(!method.equalsIgnoreCase("POST") ){
 * 
 * if(!shouldExclude((HttpServletRequest) request)) { HttpServletRequest
 * req=(HttpServletRequest) request; LoggedUser loggedInUser=(LoggedUser)
 * req.getSession().getAttribute("user"); if(loggedInUser!=null) {
 * if(daoHelper!=null) {
 * 
 * int data=daoHelper.getListJSONUrl("validurl","/IVEDA"+spath,loggedInUser.
 * getUserId()); boolean result=checkHijack(request, response); if(data==0) {
 * HttpServletResponse res = (HttpServletResponse) response;
 * res.setContentType("text/html"); PrintWriter pw = res.getWriter(); pw.
 * write("<html><head><title>Error</title></head><body><h1>You are not allowed to view this page</h1></body></html>"
 * ); }else if(result==false) { HttpServletResponse res = (HttpServletResponse)
 * response; res.setContentType("text/html"); PrintWriter pw = res.getWriter();
 * pw.
 * write("<html><head><title>Error</title></head><body><h1>Session is already active</h1></body></html>"
 * ); }else { chain.doFilter(request, response); } } } }else {
 * chain.doFilter(request, response); } } else { if
 * (spath.equals("/ConfigDetails") || spath.equals("/desktopConfigDtls") ) {
 * 
 * chain.doFilter(request, response); }else{
 * 
 * boolean result=checkHijack(request, response);
 * 
 * if(result==false) { HttpServletResponse res = (HttpServletResponse) response;
 * res.setContentType("text/html"); PrintWriter pw = res.getWriter(); pw.
 * write("<html><head><title>Error</title></head><body><h1>Session is already active</h1></body></html>"
 * ); }else {
 * 
 ...
 * getParamsFromSimpleForm((HttpServletRequest) request); }
 * 
 * 
 * 
 * StringBuffer sb = new StringBuffer("");
 * 
 * if (paramMap != null && paramMap.size() > 0) { Collections.sort(paramMap, new
 * Comparator<ParamObject>() { public int compare(ParamObject s1, ParamObject
 * s2) { return s1.getName().toLowerCase().toString()
 ...
 * fHTokenVal = paramObject.getValue(); } else { String val =
 * paramObject.getValue().replaceAll(" ", "_"); val = val.replaceAll("\\r?\\n",
 * "_"); sb.append(val); }
 * 
 * }
 * 
 ...
 * 
 * 
 * String fToken = ""; if(!fHTokenVal.equals("")) {
 * 
 * System.out.println("inside ftoken con1111:: " + fHTokenVal);
 * 
 * fToken= sb.toString().isEmpty() ? "" :
 * SecurityUtil.getMd5Hash(sb.toString());
 * 
 * }
 * 
 ...
 * 
 * res.setContentType("text/html"); PrintWriter pw = res.getWriter(); pw.
 * write("<html><head><title>Error</title></head><body><h1>Form Data Tampered</h1></body></html>"
 * );
 * 
 * }
 * 
 ...
 *//**
	 ...
	 *//*
		 ...
		 * }
		 * 
		 * private static List<ParamObject> getParamsFromSimpleForm( HttpServletRequest
		 * request) {
		 * 
		 ...
		 * 
		 * } else { for (int i = 0; i < paramValues.length; i++) { if
		 * (!("hmode".equalsIgnoreCase(paramName) && i == 0)) paramMap.add(new
		 * ParamObject(paramName .toLowerCase(), paramValues[i])); }
		 * 
		 * }
		 * 
		 * }
		 ...
		 * return paramMap; }
		 * 
		 * private static List<ParamObject>
		 * getParamsFromMultipartForm(HttpServletRequest request) {
		 * 
		 * Enumeration<String> paramNames = request.getParameterNames();
		 * 
		 * List<ParamObject> paramMap = new ArrayList<ParamObject>();
		 ...
		 * (paramValue.length() > 0)
		 * 
		 * System.out.println(paramName +"  >>  "+paramValue); paramMap.add(new
		 * ParamObject(paramName.toLowerCase(), paramValue));
		 * 
		 * } else { for (int i = 0; i < paramValues.length; i++) { if
		 * (!("hmode".equalsIgnoreCase(paramName) && i == 0)) paramMap.add(new
		 * ParamObject(paramName .toLowerCase(), paramValues[i]));
		 ...
		 * 
		 * 
		 * public boolean checkHijack(ServletRequest request, ServletResponse response)
		 * throws IOException {
		 * 
		 * HttpServletRequest incomingReq=(HttpServletRequest) request;
		 * HttpServletResponse outRes=(HttpServletResponse) response; //Session
		 * Highjacking code boolean result=false;
		 * 
		 * //11111111111111
		 * 
		 * Cookie jSessionCookie = null; Cookie[] cookies = incomingReq.getCookies();
		 * String originalUserId = ""; String sessionId = ""; String decodedSessionId =
		 * ""; String originalSessionId = ""; if (cookies != null) { for(int
		 * i=0;i<cookies.length;i++){ Cookie cookie = cookies[i];
		 * if(cookie.getName().equals("window_count_save")){ originalUserId=
		 * cookie.getValue(); }else if(cookie.getName().equals("hit_count_save")){
		 * sessionId = cookie.getValue(); byte[] decodedSessionByte =
		 * Base64.decodeBase64(cookie.getValue().getBytes()); decodedSessionId = new
		 * String(decodedSessionByte); } else if(cookie.getName().equals("JSESSIONID")
		 * || cookie.getName().equalsIgnoreCase("cdacid")){ jSessionCookie= cookie;
		 * originalSessionId = cookie.getValue(); } } }else {
		 * System.out.println("Cookies are null"); } String browserInfo =
		 * incomingReq.getHeader("User-Agent"); HttpSession session =
		 * incomingReq.getSession(false); if (null != session) { UserDetails usrInfo =
		 * (UserDetails) session.getAttribute("SessInfo");
		 * 
		 * if (null != usrInfo) { Long newUserId = usrInfo.getUserId(); String encUserId
		 * = usrInfo.getEncUserId(); String encSessionId = usrInfo.getEncSessionId();
		 * String tomcatSessId = usrInfo.getTomcatSessId(); if
		 * (StringUtils.isNotBlank(originalUserId) && StringUtils.isNotBlank(sessionId))
		 * { if ((!originalUserId.equals(encUserId)) ||
		 * (!sessionId.equals(encSessionId)) ||
		 * (!browserInfo.equals(usrInfo.getBrowserInfo())) ||
		 * (!originalSessionId.equals(tomcatSessId))) {
		 * 
		 * Cookie userIdCookie = new Cookie("window_count_save", originalUserId); Cookie
		 * sessionCookie = new Cookie("hit_count_save", sessionId);
		 * 
		 * if (jSessionCookie == null) System.out.println("jsessioncookie is null");
		 * 
		 * jSessionCookie.setValue(sessionId);
		 * jSessionCookie.setVersion(jSessionCookie.getVersion() + 1);
		 * 
		 * outRes.addCookie(userIdCookie); outRes.addCookie(sessionCookie);
		 * 
		 * result=false; }else { result=true; } }else { result=false; } }else {
