...
@Service
public class PointOfDisServiceImpl implements PointsOfDisService{
	...
	@Override
	public String ShowPointsOfDistributionDtls(HttpServletRequest request) {
		...
		String Query="select num_distribution_id, str_region_name,  dava.country_ids_name( str_country_id) countryname, str_cont_per_name, str_address, str_contact_number, str_email_id, str_cont_per_desg from distribution_mst d JOIN region_mst r ON(d.num_region_id = r.num_region_id) where d.num_isvalid = 1 and d.num_inst_id= "+ instId;
	...
	public DistributionMstDomain addDistributionMstDetail(PointsOfDisModel pointsOfDisModel, HttpServletRequest request) {
		String loogedInUserId=SecurityContextHolder.getContext().getAuthentication().getName();
		List<RegistrationDetails> userDetails = davaDao.getUserDetails(loogedInUserId);
		int instId = userDetails.get(0).getNumInstId();
